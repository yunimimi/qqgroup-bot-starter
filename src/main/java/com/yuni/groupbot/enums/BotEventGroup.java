...
import static com.yuni.groupbot.enums.BotEvent.*;

/**
 * @author zhuangwenqiang
 * @date 2024/7/10 上午11:10
 */
@AllArgsConstructor
@Getter
public enum BotEventGroup {
    GUILDS(1 << 0, Arrays.asList(
    ...
    GUILD_MEMBERS(1 << 1, Arrays.asList(
            GUILD_MEMBER_ADD, // 当成员加入时
            GUILD_MEMBER_UPDATE,  // 当成员资料变更时
            GUILD_MEMBER_REMOVE   // 当成员被移除时
    )),
    // 消息事件，仅 *私域* 机器人能够设置此 intents。
    GUILD_MESSAGES(1 << 9, Arrays.asList(
            MESSAGE_CREATE, // 发送消息事件，代表频道内的全部消息，而不只是 at 机器人的消息。内容与 AT_MESSAGE_CREATE 相同
            MESSAGE_DELETE   // 删除（撤回）消息事件
    )),
    GUILD_MESSAGE_REACTIONS(1 << 10, Arrays.asList(
            MESSAGE_REACTION_ADD,   // 为消息添加表情表态
            MESSAGE_REACTION_REMOVE // 为消息删除表情表态
    )),
    DIRECT_MESSAGE(1 << 12, Arrays.asList(
            DIRECT_MESSAGE_CREATE,  // 当收到用户发给机器人的私信消息时
            DIRECT_MESSAGE_DELETE // 删除（撤回）消息事件
    )),
    GROUP_AND_C2C_EVENT(1 << 25, Arrays.asList(
    ...
    INTERACTION(1 << 26, Collections.singletonList(
            INTERACTION_CREATE// 互动事件创建时
    )),
    MESSAGE_AUDIT(1 << 27, Arrays.asList(
            MESSAGE_AUDIT_PASS,// 消息审核通过
            MESSAGE_AUDIT_REJECT // 消息审核不通过

    )),
    //论坛事件，仅 *私域* 机器人能够设置此 intents。
    FORUMS_EVENT(1 << 28, Arrays.asList(
    ...
    AUDIO_ACTION(1 << 29, Arrays.asList(
            AUDIO_START,   // 音频开始播放时
    ...
    //此为公域的消息事件
    PUBLIC_GUILD_MESSAGES(1 << 30, Arrays.asList(
